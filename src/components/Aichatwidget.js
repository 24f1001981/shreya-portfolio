...
function getResponse(input, context, memory) {
  ...
  // Random glitch / interrupt (3%) — suppressed on first message so it's not jarring
...
export default function AIChatWidget() {
  ...
  // scroll-lock: don't auto-scroll if user has scrolled up
  ...
  const timers       = useRef([]);
  // Refs hold latest context/memory — no stale closures, no redundant state
  ...
  // Smart auto-scroll: only pull down if user hasn't scrolled up
  useEffect(() => {
    if (!userScrolled) {
  ...
  // Detect user scrolling up — stop auto-scroll; snap back when near bottom
  useEffect(() => {
  ...
  // Boot on first open; reset chat state consistently on re-open
  useEffect(() => {
    if (open && !bootDone) {
      setBooting(true);
      addTimer(() => {
  ...
  // Reset chat — clears messages and context/memory refs
  ...
  // Mark old suggestions as used when a new message is sent
        ...
        <div className="ai-panel" role="dialog" aria-label="Chat with Shreya's AI">
          ...
          <div className="ai-panel-header">
            ...
            <div className="ai-header-actions">
              {/* Reset chat button — only shown when there are messages */}
              {messages.length > 0 && (
                <button
              ...
              <div key={`msg-${i}`} className={`ai-bubble ai-bubble--${m.role}`}>
                ...
                {m.role === 'assistant' ? (
                  <div>
                    ...
                    />
                    {/* Suggestions hidden once used — prevents stale clicks */}
          ...
          <div className="ai-input-area">
            ...
            {charCount > 0 && (
              <p className={`ai-char-count ${charWarning ? 'ai-char-count--warn' : ''}`}>
                {charCount}/300
              </p>
            )}
          </div>
